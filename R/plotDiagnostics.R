...
plotDiagnostics <- function(sce, ch) {
    ...
    p_before_after <- before_after %>%
        ...
        mutate(correction =
                   factor(.data$correction, levels = c('after', 'before'))) %>%
        ggplot(aes(
            .data$value,
            color = .data$correction,
            linetype = .data$correction
        )) +
        geom_freqpoly(alpha = 1.0,
                      bins = 50,
                      linewidth = 0.8) +
